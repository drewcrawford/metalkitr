...
impl MTKTextureLoader {
    ...
    pub fn newTextureWithDataOptionsError<'p>(&mut self, data: &foundationr::NSData, options: &foundationr::NSDictionary<MTKTextureLoaderOption,NSObject>, pool: &'p ActiveAutoreleasePool) -> Result<StrongMutCell<metalr::MTLTexture>,AutoreleasedCell<'p, foundationr::NSError>> {
        unsafe {
            ...
            texture.map(|texture|
                ...
                MTLTexture::assume_nonnil(texture).assume_retained().assume_mut())
        }
    }
}
